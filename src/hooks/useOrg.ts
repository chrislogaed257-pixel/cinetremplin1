...
export function useOrgContext() {
  ...
  const myBasePositions = myId ? basePositionNamesOf(myId, profilePositions, positions) : [];
...
export function useActivePosition(myPositions: string[]) {
  ...
  useEffect(() => {
    ...
    const stored = typeof window !== "undefined" ? localStorage.getItem("cct-active-position") : null;
    setActive(stored && myPositions.includes(stored) ? stored : (myPositions[0] as string));
  }, [myPositions.join("|")]);

  const choose = (value: string) => {
    setActive(value);
    if (typeof window !== "undefined") localStorage.setItem("cct-active-position", value);
  };
