...
function DirectMessagesPage() {
  ...
  const open = useMutation({
    mutationFn: async (otherId: string) => {
      ...
      const existing = (participants.data ?? []).find(
      ...
      const { data: conv, error } = await supabase
        ...
        .insert({
          kind: "direct",
          title: `${org.profileName(org.myId)} — ${org.profileName(otherId)}`,
        })
        .select("id")
        .single();
      if (error || !conv) throw new Error(error?.message ?? "Impossible d'ouvrir la conversation");
      const { error: pErr } = await supabase.from("conversation_participants").insert([
        { conversation_id: conv.id, profile_id: org.myId },
